...
print "Files to check:/n /etc/sudoers, /etc/pam.d/common-password & common-auth, tmp files, all unit files, grub.d/(custom_10,40,more?), /etc/apt/apt.conf, /etc/apt/apt.conf.d, /etc/apt/sources.list, /etc/apt/sources.list.d"
print "Ensure all required services are running"
print "Check if the kernel or GRUB needs to be updated, then run    sudo apt dist-upgrade followed by     sudo update-grub"
